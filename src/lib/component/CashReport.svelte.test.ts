...
test('CashReport ', async () => {
	...
	await expect.element(output).toHaveTextContent('No real Placeholder (any string from var or func) Visitors: 30 Donations: 100,00 € / 3,33 € per Visitor With Replacements: 100,00 € (3,33 € per Visitor) Cash @ Start: 10,00 € Cash @ End: 30,00 € Final Cash including donations and outgoings: 130,00 € revenue (End - Start + payouts): 20,00 € payouts between Start & End: undefined (no outgoings) We earned this time (End - Start + donation): 120,00 € (! outgoings excluded).')
...
`, {normalizeWhitespace: false /* non-breaking space are regardless normalized? */})
...
`, {normalizeWhitespace: false}	)
	...
	await user.tab()
	await user.keyboard('16.50')
	await morePayouts.fill('Payout 1')
	await user.tab()
	await user.keyboard('9.95')
